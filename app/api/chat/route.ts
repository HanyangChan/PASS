...
import {fastRuleTurn,singleTurn} from '@/lib/single-gift.mjs';
...
export async function POST(request:Request){
 ...
 const fast=fastRuleTurn(input,products);
 if(fast)return json({...fast,mode:'rules_fast',notice:'빠른 선택으로 바로 반영했어요.'});
 ...
 catch{return json({error:'Gemini 응답을 받지 못했습니다. 조건은 바뀌지 않았어요. 잠시 후 다시 보내주세요.'},503);}
}
