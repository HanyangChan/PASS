...
export function GET(){return json({configured:!!config().OPENAI_API_KEY});}
export async function POST(request:Request){
 ...
 if(!OPENAI_API_KEY)return json({...singleTurn({state:input.state,issues:input.issues},input.text,products),mode:'rules',notice:'LLM 연결 대기 · 기본 해석으로 처리했습니다.'});
 try{return json({...await llmTurn(input,products,{apiKey:OPENAI_API_KEY,model:OPENAI_MODEL||'gpt-4.1-mini'}),mode:'llm',notice:'LLM으로 대화 조건을 이해했어요.'});}
