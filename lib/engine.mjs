...
// A single ranking path is shared by normal and hypothetical recommendations.
export function budgetCost(product, state) {
 const unit = product.price_krw + (state.budget.shipping_included ? product.shipping_fee_per_set_krw : 0);
 return state.budget.scope === 'total' ? unit * (state.quantity || 1) : unit;
}
export function rankProducts(products, s) {
 ...
 const items=products.filter(p=>!s.excluded_categories.includes(p.category)&&(!s.brand||p.brand===s.brand)&&(!s.packaging||p.packaging===s.packaging)&&(!s.delivery_by||p.arrival_date<=s.delivery_by)&&p.stock_sets>=q).map(p=>({...p,total:q*(p.price_krw+(s.budget.shipping_included?p.shipping_fee_per_set_krw:0)),score:s.preferences.reduce((v,x)=>v+(x===`category:${p.category}`?3:x==='unsweetened'&&p.sweetness==='unsweetened'?2:x==='less_sweet'?({unsweetened:2,naturally_sweet:1,sweet:0}[p.sweetness]||0):0),0)})).filter(p=>s.budget.amount_krw===null||budgetCost(p,s)<=s.budget.amount_krw).sort((a,b)=>b.score-a.score||a.price_krw-b.price_krw||a.id.localeCompare(b.id));
 return {status:items.length?'ready':'no_match',message:items.length?'현재 조건에 맞는 상품을 골랐어요.':'조건에 맞는 상품이 없어요. 예산이나 제외 조건을 조정해 주세요.',items};
}
export function recommend(products,s){const result=rankProducts(products,s);return {...result,items:result.items.slice(0,3)}}
